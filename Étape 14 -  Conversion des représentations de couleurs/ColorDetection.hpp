...
public:
    ...
    {
        cv::Mat tmp(1, 1, CV_8UC3); // Crée une matrice temporaire
        tmp.at<cv::Vec3b>(0, 0) = cv::Vec3b(blue, green, red); // Définit la couleur cible
        cv::cvtColor(tmp, tmp, cv::COLOR_BGR2Lab); // Convertit la couleur en Lab
        target = tmp.at<cv::Vec3b>(0, 0); // Récupère la couleur cible
    }
